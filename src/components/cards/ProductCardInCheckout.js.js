...
function ProductCardInCheckout({p}) {
    const colors=["Black",'Silver','White','Blue','Red','Brown','Pink','Many'];
...
if(product._id === p._id){
 cart[i].color = e.target.value
  ...
  //  console.log("update cart color",cart);
  ...
  dispatch({
     ...
     const handleCount=(e)=>{
         ...
         console.log("Avalable products", p.quantity);
         if(count > p.quantity){
         ...
         <tr>
             <td>
                 <div style={{width:"100px",height:"auto"}}>
                     {p.images.length ?(<ModalImage small={p.images[0].url} large={p.images[0].url}/>):(<ModalImage small={laptopDefaultImage} large={laptopDefaultImage}/>)} 
                 </div>
...
{colors.filter((c)=> c !== p.color).map((c)=>{
    return <option key={c} value={c}>{c}</option>
             ...
             <td  >
                 <input type="number" className="form-control" value={p.count} onChange={handleCount}/>
             </td>
